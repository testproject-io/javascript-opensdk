...
// Imported from the appium-uiautomator2-driver project
// https://github.com/appium/appium-uiautomator2-driver

/* eslint-disable */

import { CssSelectorParser } from 'css-selector-parser';
import { errors } from 'selenium-webdriver';
...
function parseAttr(cssAttr) {
  if (cssAttr.valueType && cssAttr.valueType !== 'string') {
    throw new TypeError(
      `'${cssAttr.name}=${cssAttr.value}' is an invalid attribute. ` +
...
function parsePseudo(cssPseudo) {
  if (cssPseudo.valueType && cssPseudo.valueType !== 'string') {
    throw new Error(
      `'${cssPseudo.name}=${cssPseudo.value}'. ` +
...
function parseCssRule(cssRule) {
  ...
  if (nestingOperator && nestingOperator !== ' ' && nestingOperator !== '>') {
    throw new Error(
      `'${nestingOperator}' is not a supported combinator. ` +
...
function parseCssObject(css) {
  switch (css.type) {
    ...
    default:
      ...
      throw new Error(
        `iOS Class Chain does not support '${css.type}' css. Only supports 'rule', 'ruleSet', 'selectors'`
      );
  }
}
...
export default cssToIosClassChainSelector;
